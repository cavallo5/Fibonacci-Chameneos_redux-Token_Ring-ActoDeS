  ...
  **/
  private void configure(final String u)
  {
    try
    {
      ...
      this.dealer = this.context.createSocket(ZMQ.REQ);

      this.dealer.connect(url);

      this.queue = this.context.createSocket(ZMQ.DEALER);
    }
    catch (Exception e)
    {
      ErrorManager.kill(e);
  ...
  private final class Broker implements Runnable
  {
    ...
    private Broker(final String u)
    {
      ...
      this.socket  = this.context.createSocket(ZMQ.REP);
  ...
  protected Connection create(final Reference d)
  {
    try
    {
      return new JeroMqConnection(d, this.context.createSocket(ZMQ.DEALER));
    }
    catch (Exception e)
    {
